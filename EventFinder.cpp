...
{
  char t;	
  std::cout << "Im here EventFinder" << std::endl;	
  //Begin of exec function
  if (auto timeWindow = dynamic_cast <const JPetTimeWindowMC* const > (fEvent)) {
    const unsigned int nHits = timeWindow->getNumberOfEvents();
    unsigned int count = 0;
    while(count<nHits){
      auto hit = timeWindow->getMCHit<JPetMCHit>(count);
      //auto hit = dynamic_cast<const JPetMCHit&>(timeWindow->operator[](count));
      std::cout << "Im taking next hit" << std::endl;	
      std::cout <<hit.getGenGammaMultiplicity()<< std::endl;
      std::cin >> t;
      count++;
    }
  }
  if (auto timeWindow = dynamic_cast<const JPetTimeWindow* const>(fEvent)) {
	  ...
	  std::cout << "Im inside EventFinder MC widnow loop" << std::endl;	
...
{
  std::cout <<"Im in buildEvents starting"<< std::endl;
  ...
  while(count<nHits){
    ...
    std::cout <<"!!!!!!!!!!!!!!!!!"<< std::endl;
    std::cout <<"Im in buildEvents inside while"<< std::endl;
    //std::cout <<hit.getGenGammaMultiplicity()<< std::endl;
